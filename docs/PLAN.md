...
**Custom domain added post-launch — `jarcodes.dev`, live and verified. Recording this here because Codex only sees what's in this file, not what happened in Cloudflare/Vercel/GitHub's own dashboards.**
- Registered at Cloudflare. Real issue hit and fixed during setup: Cloudflare defaults new DNS records to Proxied, which broke Vercel's SSL certificate issuance (manifested as an SSL handshake failure). Fixed by switching both the apex and `www` CNAME records to DNS-only.
- `www.jarcodes.dev` is the Production/canonical domain in Vercel; bare `jarcodes.dev` 308-redirects to it.
- `NEXT_PUBLIC_SITE_URL` set directly in Vercel's environment variables — this makes it the canonical URL everywhere (sitemap, RSS, OG tags) with zero code changes, since `lib/site.ts`'s fallback chain already checks this first.
- **The GitHub OAuth App's Homepage URL and callback URL have been updated to `jarcodes.dev`** — confirmed directly with Jethro, not assumed. Admin sign-in depends on this matching exactly; if a future domain change ever happens, this is the step most likely to be forgotten since it lives entirely outside the repo.
- `README.md`'s live-site link was updated to match this domain — **verify it's actually committed**, it was still showing as a local uncommitted change as of the Milestone 10 check.
...
**Real deviation from spec, found during verification — decided, fix before Milestone 10.** The "Rendering" section above says post pages should "stay statically generated, with `revalidatePath` called on publish." The actual build output showed `/`, `/writing`, and `/writing/[slug]` all marked `ƒ` (dynamic, server-rendered per request) instead of `○`/`●` (static) — every page visit was querying Supabase live, not just after a publish/edit. **Decision: fix it properly**, not accept as a pragmatic simplification — static + on-demand revalidation is strictly better here (faster, cheaper, more resilient) with no real downside once implemented, and this read as an implementation shortcut rather than a deliberate tradeoff. Codex should implement real static generation with `revalidatePath`-driven invalidation, matching what was originally specced, before Milestone 10 starts.

**Fixed and verified (commit `360a973`).** Re-ran the build myself — `/` and `/writing` are now `○`, all 4 posts `●`, zero `ƒ` outside `/admin/*` and the auth API route (correctly still dynamic, they need live sessions). `lib/content.ts` wraps the Supabase read in `unstable_cache` tagged `"content-published-posts"`; `app/admin/actions.ts` calls a matching `updateTag("content-published-posts")` alongside the existing `revalidatePath` calls. Note: `updateTag` (not the older `revalidateTag`) — plausible newer-Next.js-16 API, not confidently verifiable against training data, but the build compiling clean with this exact import is strong evidence it's valid for this Next.js version. Live-checked `/writing` on production again — correct content, console clean, no leftover test data. **Not independently re-verified: the full interactive publish → appears → revert → disappears cycle** — that requires real GitHub OAuth sign-in as the allowlisted account, which isn't something checkable from here. Resting on Codex's reported test for that specific piece, not on independent reproduction — everything else was verified directly.
...
**Verified complete (commit `df3e230`)** — checked directly, not assumed from the report. Preview and the public post page were confirmed to genuinely share one render pipeline: both call into the new `lib/mdx.tsx` (`renderMdxToHtml` for preview, `MdxContent` for the public page), same `mdxComponents`, same `rehypePrettyCodePlugin` config — not two approximate copies. That refactor also added `rehype-sanitize`, which wasn't present before, retroactively improving the already-published Milestone 5/9 content too. Autosave, the localStorage draft-recovery backup, and the unsaved-changes guard (covering both browser `beforeunload` *and* in-app client-side navigation, which most implementations miss) all exceed what was strictly asked for. Image upload validates type and file size server-side, not just via the client's `accept` attribute, with matching limits enforced at the Supabase Storage bucket level too. `npm run lint` and `npm run build` both re-run clean; the static/dynamic split from the previous fix is still intact, with only `/admin/*` and the new `/api/admin/*` routes correctly dynamic. **One minor gap, not urgent:** the security checklist's "cap dimensions" isn't implemented — only file size (MB) is capped, not pixel width/height, so a small-file-size but huge-resolution image could still pass.
