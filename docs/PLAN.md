...
10. **Commit at the end of every milestone and every approved checkpoint, with a real descriptive message — not just at scaffold time.** Milestone 1's `git init` + push was a one-time setup step, never an ongoing instruction, and this plan never established one until now: as of the Milestone 4 checkpoint approval, the repo still has only its original `create-next-app` commit — every one of Milestones 2 through 4b exists solely as uncommitted working-tree changes, with no rollback point anywhere in that work. Going forward, commit at each milestone boundary (and after each 🔍 checkpoint gets approved) rather than letting changes accumulate indefinitely.
